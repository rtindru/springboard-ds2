...
features = ['x', 'y', 'hour', 'weekday', 'day', 'month', 'year']
features_rf = features + ['l1', 'l2', 'l3', 'l4', 'l5', 'l6', 'l7', 'l8', 'l9', 'l10']
...
def process_one_cell(df_train, df_test, grid_id, th):
    ...
    pred_labels = le.inverse_transform(np.argsort(y_pred, axis=1)[:, ::-1][:, :10])
...
class MultiPredictionModel(object):
    def __init__(self, df, features, xsize=0.5, ysize=0.5, xslide=0.25, yslide=0.25, xcol='x', ycol='y',
...
def run(xsize, ysize, xstep, ystep, n_estimators, n_jobs, df_train, df_test):
    try:
    ...
    pred_model = MultiPredictionModel(df_train, features_rf, xsize, ysize, xstep, ystep, 'x', 'y', n_estimators, n_jobs)
    print 'Init done'
...
if __name__ == '__main__':
    ...
    result_train_df = process_grid(df_train, df_train.drop('place_id', 1), th, n_cell_x * n_cell_y)
    result_test_df = process_grid(df_train, df_test, th, n_cell_x * n_cell_y)

    import pdb
    pdb.set_trace()

    df_train2 = df_train.merge(result_train_df)
    df_test2 = df_test.merge(result_test_df)
    ...
    # n_jobs = args.n_jobs
    xsize, ysize, xslide, yslide, n_estimators, n_jobs = 0.5, 0.25, 0.5, 0.25, 30, -1

    print run(xsize, ysize, xslide, yslide, n_estimators, n_jobs, df_train2, df_test2)
