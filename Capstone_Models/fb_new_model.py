...
xytime_bins_file_name_str = 'timexy_model_xybin_{}.pkl'
...
class MultiPredictionModel(object):
    ...
    def feature_engineering(self, df):
        ...
        self.xytime_bins = df.xytime_bin.unique()
    ...
    def train(self):
        ...
        if not train_xytime_bin:
            return

        for bin_id in sorted(self.xytime_bins):
            file_name = xytime_bins_file_name_str.format(bin_id)
            if os.path.isfile(file_name):
                print 'Already Trained'
                continue

            print 'Training model: {} of {}'.format(bin_id, max(self.xytime_bins))

            df = self.df
            wdf = df[df.xytime_bin == bin_id]
            X = wdf[self.features]
            y = wdf.place_id
    ...
    def load_xytime_model(self, xytime_bin):
        try:
            file_name = xytime_bins_file_name_str.format(xytime_bin)
            model = joblib.load(file_name)
            return model
        except:
            None
    ...
    def predict(self, df, test=False):
        ...
        for bin_id in xy_bins:
            ...
            if len(wdf) == 0:
                continue
            if model is None:
                print 'XY Model missing for bin: {}'.format(bin_id)
                print 'Affects {} row_ids'.format(len(wdf))
                for i in xrange(len(wdf)):
                    row_id = wdf.row_id.iloc[i]
                    self.result_set_1[row_id] = zip([0]*3, [0]*3)
                continue
            ...
            for i in xrange(len(wdf)):
                ...
                self.result_set_1[row_id] = zip(places, indices)

            model = None
            del model
            gc.collect()

        xytime_bins = sorted(df.xytime_bin.unique())
        for bin_id in xytime_bins:
            print 'Predicting Model: {} of {}'.format(bin_id, max(xy_bins))
            model = self.load_xytime_model(bin_id)
            wdf = df[df.xy_bin == bin_id]

            if len(wdf) == 0:
                continue
            if model is None:
                print 'TimeXY Model missing for bin: {}'.format(bin_id)
                print 'Affects {} row_ids'.format(len(wdf))
                for i in xrange(len(wdf)):
                    row_id = wdf.row_id.iloc[i]
                    self.result_set_2[row_id] = zip([0]*3, [0]*3)
                continue
            ...
            for i in xrange(len(wdf)):
                ...
                indices = np.argsort(predictions[i])[-3:][::-1]
                places = model.classes_[indices]
                self.result_set_2[row_id] = zip(places, indices)

            model = None
            del model
            gc.collect()

        return self.get_comined_results(test)

    def get_comined_results(self, test=False):
        keys = sorted(set(self.result_set_1.keys() + self.result_set_2.keys()))
        for row_id in keys:
            results_1 = self.result_set_1.get(row_id, [])
            results_2 = self.result_set_2.get(row_id, [])
            # combined = sorted(results_1 + results_2, key=itemgetter(1), reverse=True)
            combined = results_2
            place_list = []
            for place_id, prob in combined:
                if place_id not in place_list:
                    place_list.append(place_id)
                if len(place_list) == 3:
                    break

            self.result_set[row_id] = place_list
